...
class MainVC: UIViewController {
    ...
    @IBOutlet weak var startBtnWidthConstraints: NSLayoutConstraint!
    @IBOutlet weak var labelCenterYConstraints: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        ...
        Timer.scheduledTimer(withTimeInterval: 2, repeats: false) { (timer) in
            //Helper.setLottieAnimation(splach_view: self.splach_view, self.lottie_json)
...
extension MainVC: UIGestureRecognizerDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.9, repeats: false) { (timer) in
            self.animateStartButtonCenter()
            self.shrinkStartButton()
        }
